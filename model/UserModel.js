...
var FoodieModel = /** @class */ (function (_super) {
    ...
    FoodieModel.prototype.createSchema = function () {
        ...
        this.schema.index({ userID: 1 }, { password: null }, { emailAddress: null }, { reviewList: null }, { unique: true });
    };
...
var AdminModel = /** @class */ (function (_super) {
    ...
    AdminModel.prototype.getAllFoodies = function (response, foodieType) {
        if (foodieType !== 1) {
            ...
            var query = this.model.find({ userType: foodieType });
            query.exec(function (err, foodieList) {
                response.json(foodieList);
            });
        }
    };
    AdminModel.prototype.getAllRestaurantOwners = function (response, ownerType) {
        if (ownerType !== 2) {
            console.error("Wrong user type");
        }
        else {
            var query = this.model.find({ userType: ownerType });
            query.exec(function (err, ownerList) {
                response.json(ownerList);
            });
        }
    };
    AdminModel.prototype.getAllAdmins = function (response, adminType) {
        if (adminType !== 2) {
            console.error("Wrong user type");
        }
        else {
            var query = this.model.find({ userType: adminType });
